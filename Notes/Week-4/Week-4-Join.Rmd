...
finalPlot <- 
  ...
  geom_point( aes(size=urbanFrac, color=cut(log10(population),breaks=7) ) ) +
  scale_size_area() 
...
To get the email to show up as a link, enclose it in braces (`<` and `>`)
...
What will the glyph-ready data look like to produce the plot?  Sketch out the table putting in a case or two of made-up data in the right form.

Take a picture of your sketch and send it to yourself so that you can put a sketch of the table in your Rmd document.
...
```
NewName <- select( InputData, newname=oldname,
...
You'll have to translate the `WorldCities` country into a form compatible with `CountryData` and `CountryCentroids`, or *vice versa*.  To do this, you may find the `Codes` and `Synonyms` data tables useful.  Load them with this command.
