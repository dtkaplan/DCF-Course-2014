---
title: "DCF Week 5 Warm-Up Exercises"
output:
  pdf_document:
    fig_height: 3
    fig_width: 5
...
Note that the `DBA` variable contains the name of the restaurant.
...
Clean up the `Restaurants` data.

* Remove quotes inside strings.
* Turn Score into a number
* Turn CURRENTGRADE into a single letter.  Replace non-letters with NA.
* Turn ACTION into a single letter.  NA's for those that are non-letters.
* Turn BORO into a single number.  NA's for those not 1:5.
